...
const Experience: React.FC<{ data: TimelineEntry[] }> = ({ data }) => {
  ...
  return (
    <section id='experience' className='pt-36 pb-4 md:pt-44'>
      <div className='mx-auto max-w-5xl space-y-8 px-6 md:space-y-16'>
        ...
        <div className='w-full' ref={containerRef}>
          <div ref={ref} className='relative space-y-10 md:space-y-20'>
            {data.map((item) => (
              <div key={item.title} className='flex justify-start md:gap-10'>
                <div className='sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full'>
                  ...
                  <h3 className='hidden md:block text-xl md:pl-20 md:text-2xl font-bold font-mono'>
                    {item.title}
                  </h3>
                </div>

                <div className='relative pl-20 pr-4 md:pl-4 w-full'>
                  <h3 className='md:hidden block text-2xl mb-4 text-left font-bold font-mono'>
                    {item.title}
                  </h3>
                  {item.content}
                </div>
